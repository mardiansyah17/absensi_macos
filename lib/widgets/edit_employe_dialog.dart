...
class EditEmployeDialog extends StatefulWidget {
  ...
  @override
  State<EditEmployeDialog> createState() => EditEmployeDialogState();
}

class EditEmployeDialogState extends State<EditEmployeDialog> {
  late final TextEditingController _name = TextEditingController(
    text: widget.employe.name,
  );
  late final TextEditingController _code = TextEditingController(
    text: widget.employe.nip,
  );
  late final TextEditingController _email = TextEditingController(
    text: widget.employe.email,
  );
  late Department? department = widget.employe.department;
  ...
  void _onSave() async {
    ...
    try {
      final newEmp = Employe(
        ...
        email: email,
        department:
            widget.departments.firstWhere((d) => d.id == department?.id),
      );
      await RemoteDatasource().updateEmploye(
        widget.employe.id,
        {
          'name': newEmp.name,
          'nip': newEmp.nip,
          'email': newEmp.email,
          'departmentId': newEmp.department!.id,
        },
      );
            ...
            children: [
              ...
              DropdownButtonFormField<String?>(
                ...
                onChanged: (v) => setState(() {
                  department = widget.departments.firstWhere(
                    (d) => d.id == v,
                    orElse: () => Department(id: '', name: 'Tidak ada'),
                  );
                }),
                decoration:
              ...
              TextField(
                  ...
                  decoration: _inputDecoration()
                      .copyWith(labelText: 'Kode (opsional)')),
              const SizedBox(height: 12),
              TextField(
                  controller: _email,
                  decoration: _inputDecoration()
                      .copyWith(labelText: 'Email (opsional)')),
